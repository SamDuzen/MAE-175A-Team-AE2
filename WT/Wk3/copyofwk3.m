...
function plotData(x,y,z)
    ...
    n_y = length(Cl_y);
    n_z = length(Cl_z);
    ReyNum_x = (linspace(1,50,n_x))*1.1963*3.5/18.13e-06;
    ReyNum_y = (linspace(1,50,n_y))*1.1963*3.5/18.13e-06;
    ReyNum_z = (linspace(1,50,n_z))*1.1963*3.5/18.13e-06;

figure(5)
hold on
plot(ReyNum_x,Cl_x,'-r')
plot(ReyNum_y,Cl_y,'-m')
plot(ReyNum_z,Cl_z,'-b')
hold off
title('C_l over Reynold''s Number at Various AOA')
grid('on')
xlabel('Reynold''s Number')
ylabel('C_l')
legend('V=20m/s', 'V=35m/s', 'V=50m/s')
end
